...
### 7a. Chrome stream fetch - SABR -> CDN -> direct URL -> watch-page scrub-capture

On Chrome, [`download-worker/main.ts`](src/entrypoints/download-worker/main.ts) is the actual download engine running inside the offscreen worker iframe. It walks a four-layer fallback chain, each layer only invoked when the one above returns no usable bytes:

**1. SABR** ([`sabr-downloader.ts`](src/entrypoints/background/download/sabr-downloader.ts)) speaks YouTube's internal adaptive streaming protocol via [`googlevideo`](https://npm.im/googlevideo), constructing protobuf requests that the CDN accepts as a real player session. [`createSabrStallTimer`](src/entrypoints/background/download/sabr-stall-timer.ts) aborts and yields to CDN if no bytes arrive within 5 seconds, or if progress stalls for 10 seconds. Because the service worker can't set `Origin` headers (Chrome strips them), a `declarativeNetRequest` rule in [`wxt.config.ts`](wxt.config.ts) rewrites the header at the network layer on every outgoing request to `googlevideo.com`.

**2. CDN** ([`cdn-downloader.ts`](src/entrypoints/background/download/cdn-downloader.ts)) - plain HTTPS fetches with byte-range support so a dropped connection resumes from where it left off rather than restarting.

If both come back empty, [`download-worker/main.ts`](src/entrypoints/download-worker/main.ts) decides one of two follow-ups based on the request shape and posts back to the offscreen host, which forwards to the background:

**3. Direct CDN URL** ([`tryDirectUrlDownload`](src/entrypoints/background/download/download-fallback-chain.ts)) - audio-only with a `resolvedAudioUrl` only. The worker posts `worker-needs-direct-url`; the background hands the URL straight to `browser.downloads.download` and skips muxing entirely. If even this fails (transient block, expired signature), it falls through to layer 4.

**4. Watch-page scrub-capture** ([`downloadViaWatchPage`](src/entrypoints/background/download/iframe-downloader.ts) + [`sourcebuffer-capture`](src/entrypoints/sourcebuffer-capture/sourcebuffer-capture-patches.ts)) - the last resort, triggered by `worker-needs-fallback` or a layer-3 failure. [`handleIframeFallback`](src/entrypoints/background/download/sabr-attempt.ts) opens a hidden `<iframe src="https://youtube.com/watch?v={id}&ytdl=1&mute=1&autoplay=1">` inside the offscreen document. The MAIN-world content script patches `MediaSource.addSourceBuffer` and `SourceBuffer.appendBuffer` on the iframe's `HTMLMediaElement` prototype to siphon every video and audio segment as YouTube's own player decodes the stream into its media element. Up to two fresh-iframe retries before reporting failure. Captured bytes flow back through the same offscreen accumulator pipeline, so muxing is identical.

As each chunk arrives from any layer, the worker (layers 1-2) or the iframe (layer 4) feeds it into [`accumulator.ts`](src/entrypoints/offscreen/stream/accumulator.ts), which sequences the chunks. When all chunks for a stream have arrived, [`end-handler.ts`](src/entrypoints/offscreen/stream/end-handler.ts) marks the stream complete and, once both video and audio streams are finished, hands everything to the processing pipeline.
...
**Delivery routing inside BG**: [`progress-fetch.ts`](src/entrypoints/background/download/progress-fetch.ts) probes for tabs-API availability at runtime via `typeof document === "undefined" || browser.runtime.getURL("").startsWith("moz-extension://")` - if true (Chrome service worker, Firefox background event-page, Firefox offscreen iframe), `UpdateDownloadProgress` goes directly to the tab; otherwise (Chrome offscreen document, Chrome download-worker iframe) it routes via `ForwardProgressUpdate` to the background, which has the `tabs` API, writes the `DownloadProgressEntry` to `statusProgressItem`, and forwards on. FFmpeg muxing progress always originates in the offscreen document and uses `PipelineProgress`, which `pipeline-handlers.ts` re-broadcasts unconditionally. `sendProgressUpdate` coalesces per-`videoId` at 100 ms so the watch tab's hover/tooltip events don't get starved by chunk-rate fan-out, and threads `downloadedBytes`/`totalBytes`/`bytesPerSecond` (a 2-second sliding window) through to the popup readout.
