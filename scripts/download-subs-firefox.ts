/**
 * Firefox equivalent of download-subs-chrome.ts.
 *
 * Clicks a download button on the subscriptions page via Firefox RDP,
 * then polls until the label flips to "Downloaded"/"Open"/"Download again"
 * or "Failed" (or times out).
 *
 * Usage:
 *   npx tsx scripts/download-subs-firefox.ts [videoId]
 *
...
const MAX_POLLS = 36;
const requestedVideoId = process.argv[2];
...
const sub = tabs.find(t => t.url?.includes("subscriptions"));
if (!sub) {
  console.error("No subscriptions tab open in Firefox.");
  rdp.destroy();
  process.exit(1);
}
console.log("Found tab:", sub.url);

const targetResponse = await rdp.request(sub.actor, "getTarget");
...
const selector = requestedVideoId
  ? `JSON.stringify((() => {
       const items = [...document.querySelectorAll('[data-ytdl-grid-item=\"${requestedVideoId}\"]')];
       const item = items[0];
       const btn = item?.querySelector('yt-button-view-model button');
       return { videoId: item?.dataset?.ytdlGridItem, label: btn?.getAttribute('aria-label')?.slice(0,80), total: items.length };
     })())`
  : `JSON.stringify((() => {
       ...
       const item = items.find(el => {
         ...
         return btn && !label.startsWith('cancel') && !label.includes('failed') && !label.includes('retry') && !label.includes('downloaded') && !label.includes('download again');
       }) ?? items[0];
       const btn = item?.querySelector('yt-button-view-model button');
       return { videoId: item?.dataset?.ytdlGridItem, label: btn?.getAttribute('aria-label')?.slice(0,80), total: items.length };
     })())`;
const gridJson = await evalJs(rdp, consoleActor, selector);
console.log("Grid probe:", gridJson);
const grid = JSON.parse(gridJson as string);
if (!grid.videoId) {
...
for (let i = 0; i < MAX_POLLS; i++) {
  ...
  const stateJson = await evalJs(rdp, consoleActor, `
    JSON.stringify((() => {
      const item = document.querySelector('[data-ytdl-grid-item="${grid.videoId}"]');
      const btn = item?.querySelector('yt-button-view-model button');
      const progress = item?.parentElement?.querySelector('[data-ytdl-progress]')?.querySelector('tp-yt-paper-progress');
      return {
  ...
  if (label.includes("deleted") || label.includes("failed") || label.includes("error")) {
